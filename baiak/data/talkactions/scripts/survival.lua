...
--------------------------------------------------------------------------------
--[[CONFIG WORDS]]--
local wordsPlayer = "!goEvent" -- o que o god tem que falar.
local wordsGm = "!openEvent" -- o que o player tem que falar.
--[[CONFIG NAME]]--
...
local posPlayer = {x= 105, y= 302, z= 7} -- Posição para onde o player será teleportado.
--[[CONFIG "GOD"]]--
local storageGlobal = 17892 -- storage global.
local time = 2 -- tempo para acabar o evento em minutos.
--[[RECOMPENSA]]--
...
elseif(words == wordsGm) then
 if getPlayerGroupId(cid) >= 4 then
     if getGlobalStorageValue(storageGlobal) <= 0 then
      ...
      doPlayerSendCancel(cid, "Apenas o [God] Pode Usar essa função")
 end
end
--------------------------------------------------------------------------------
 return true
